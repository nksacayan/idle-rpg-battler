...
var battle_main: MultiplayerBattle
...
func switch_to_battle_scene() -> void:
	var ally_battle_team := CharacterManagerAutoload.ally_battle_team_data
	if ally_battle_team.is_empty():
		push_warning("Tried to start battle with an empty team")
		return
	battle_main.setup(ally_battle_team)
